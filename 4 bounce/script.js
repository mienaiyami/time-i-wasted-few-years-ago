...
function component(x,y,w,h,color,type,gravity,bounce){
    ...
    this.newPos = function(){
        ...
        this.bottomReached(myGameArea.canvas);
        infoP.innerHTML = " location x: "+this.x+" y : "+ this.y+ "<br> speed x: " + pieceBall.speedX+ "<br> speed y: " + pieceBall.speedY+"<br> gravity speed: " + pieceBall.gravitySpeed+"<br>frame number : <span class='lolp'></span>";
    }
    this.bottomReached = function(obj){
        var left = this.w;
        var top = this.h;
        var right = obj.width - this.w;
        var bottom = obj.height - this.h;
        if(this.x <= left){
            this.x = left;
            for (let i = 0; i < 5; i++) {
                if(this.speedX == "-0."+i){
...
function updateGameArea(){
    ...
    keyCtrl();
    function keyCtrl(){
        if (myGameArea.Keys && myGameArea.Keys[80]) {myGameArea.stop(); console.log("lol") } //p
        if (myGameArea.Keys && myGameArea.Keys[79]) {myGameArea.start() } //o doesnt work prop
    }
    pieceBall.newPos();
