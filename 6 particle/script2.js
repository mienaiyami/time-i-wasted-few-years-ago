...
function component(x, y, w, h, color, type, gravity, bounce) {
    ...
    this.update = function () {
        ...
        if (this.type == "main") {
            context.beginPath();
            this.main = new Path2D();
            this.main.arc(this.x, this.y, this.width, 0, 2 * Math.PI);
            context.fillStyle = this.color;
            context.fill(this.main);
        }
        if (this.type == "obst") {
            const obst = new Path2D();
            obst.rect(this.x, this.y, this.width, this.height);
            context.fillStyle = this.color;
            context.fill(obst);
        }
    };
